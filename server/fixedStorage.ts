...
export class MemStorage implements IStorage {
  ...
  private initializeSampleData() {
    ...
    this.users.set(1, { id: 1, username: "Sarah M.", password: "password123" });
    this.users.set(2, { id: 2, username: "Mike T.", password: "password123" });
    this.users.set(3, { id: 3, username: "David L.", password: "password123" });
    this.currentUserId = 4;

    // Get the users to use in requests
    const user1 = this.users.get(1)!;
    const user2 = this.users.get(2)!;
    const user3 = this.users.get(3)!;

    // Create sample requests with proper types
    this.requests.set(1, {
      id: 1,
